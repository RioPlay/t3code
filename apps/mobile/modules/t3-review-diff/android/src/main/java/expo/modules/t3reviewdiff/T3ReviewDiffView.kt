...
class T3ReviewDiffView(context: Context, appContext: AppContext) : ExpoView(context, appContext) {
  ...
  // Custom-scrolled canvas cannot host SwipeRefreshLayout nested scrolling, so pull
  // distance is tracked in the same touch path that drives verticalOffset (matches iOS
  // UIRefreshControl contract: event on pull, spinner driven by JS `refreshing` prop).
  private val pullThresholdPx = 72f * resources.displayMetrics.density
  private var pullDistancePx = 0f
  private var isRefreshing = false
  private val refreshIndicator =
  ...
  init {
    // ExpoView is a LinearLayout; overlay the spinner via an intermediate FrameLayout
    // so pull-to-refresh chrome does not stack as a second LinearLayout child.
    val contentRoot = FrameLayout(context)
    ...
    val indicatorSize = (36f * resources.displayMetrics.density).toInt()
    contentRoot.addView(
      refreshIndicator,
      FrameLayout.LayoutParams(indicatorSize, indicatorSize, Gravity.TOP or Gravity.CENTER_HORIZONTAL).apply {
        topMargin = (12f * resources.displayMetrics.density).toInt()
      },
    )
    addView(
      contentRoot,
  ...
  /** Driven from JS: false once reload completes dismisses the spinner (iOS parity). */
    ...
    when (event.actionMasked) {
      ...
      MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
        if (event.actionMasked == MotionEvent.ACTION_UP) {
          if (axis == DragAxis.VERTICAL && maybeTriggerPullToRefresh()) {
            // Refresh armed — skip fling so content stays pinned at top.
          } else {
            velocityTracker?.computeCurrentVelocity(1000)
            if (axis == DragAxis.VERTICAL) {
              val velocity = -(velocityTracker?.yVelocity ?: 0f).toInt()
              if (abs(velocity) >= minimumFlingVelocity && pullDistancePx <= 0f) {
  ...
  /**
   * deltaY uses the existing scroll convention: positive scrolls content up
   * (finger moves up). At offset 0, negative delta is overscroll-down → pull.
   */
  private fun applyVerticalDrag(deltaY: Int) {
    if (deltaY == 0) return

    val atTop = canvasView.verticalOffset() <= 0
    if (pullDistancePx > 0f || (atTop && deltaY < 0 && !isRefreshing)) {
      // Rubber-band pull when pinned to top (or releasing an existing pull).
      pullDistancePx = (pullDistancePx - deltaY).coerceAtLeast(0f)
      if (pullDistancePx == 0f && deltaY > 0) {
  ...
  private fun maybeTriggerPullToRefresh(): Boolean {
    if (isRefreshing || pullDistancePx < pullThresholdPx) {
      pullDistancePx = 0f
      updateRefreshIndicator()
      return false
    }
    ...
    isRefreshing = true
    updateRefreshIndicator()
    onPullToRefresh(emptyMap())
    return true
  }

  private fun updateRefreshIndicator() {
    val show = isRefreshing || pullDistancePx > 0f
    refreshIndicator.visibility = if (show) VISIBLE else GONE
    refreshIndicator.alpha =
      when {
        isRefreshing -> 1f
        pullDistancePx <= 0f -> 0f
        else -> (pullDistancePx / pullThresholdPx).coerceIn(0.25f, 1f)
      }
  }
