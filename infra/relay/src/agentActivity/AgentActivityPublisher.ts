  ...
  const publishForDeliveryUser = Effect.fnUntraced(function* (input: {
    ...
    const deliveriesByTarget = yield* Effect.forEach(
      ...
      (target) =>
        Effect.all(
          [
            // Live Activity / rich activity surface: iOS only. Android targets
            // still receive FCM via sendForTarget (ongoing notification path).
      ...
      const { activeStates, targets } = yield* Effect.all(
        {
          ...
          targets: mobileDeliveries.listTargets({ userId: input.userId }),
        },
        { concurrency: 2 },
      );
      const target = targets.find((row) => row.device_id === input.deviceId) ?? null;
      // Live Activity replay is iOS-only; Android has no activity push token path.
