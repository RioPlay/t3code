...
const manifestPath = resolve("apps/mobile/.maestro/screenshot-tour/manifest.json");
...
const metaPath = join(outputDir, "run-meta.json");
let runMeta = {};
try {
...
writeFileSync(join(outputDir, "validation-prompt.txt"), `${promptLines.join("\n")}\n`);

console.log(`Wrote ${join(outputDir, "report.json")}`);
console.log(`Wrote ${join(outputDir, "validation-prompt.txt")}`);
